// Targeted gameplay verify for WHACK MOLE.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/mole.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[mole gameplay]');
const g = RA.games.mole;
g.init();
RA.hideOverlay();
g.onStart();
...
// Mole: tap to start, then tap random holes
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 200 }); }
  if (g.debug().started && !g.debug().over) {
    if (i % 5 === 0) {
      const holes = g.debug().holes;
      if (holes) {
        const idx = i % holes.length;
        if (holes[idx] && holes[idx].mole) {
          RA.input.taps.push({ x: 180, y: 200 + idx * 30 });
        }
      }
    }
  }
...
check('holes' in dbg, 'debug() exposes "holes"');
...
if (fail) { console.error('\n\nMOLE GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nMOLE GAMEPLAY PASS');
