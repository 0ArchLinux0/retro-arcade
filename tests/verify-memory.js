// Targeted gameplay verify for MEMORY MATCH.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/memory.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[memory gameplay]');
const g = RA.games.memory;
g.init();
RA.hideOverlay();
g.onStart();
...
// Memory: tap to start, then tap cards
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 200 }); }
  if (g.debug().started && !g.debug().over) {
    if (i % 10 === 0) {
      // tap card 0
      const cards = g.debug().cards;
      if (cards && cards[0]) RA.input.taps.push({ x: cards[0].x + cards[0].w / 2, y: cards[0].y + cards[0].h / 2 });
    }
  }
  g.update(dt);
  g.draw(RA.ctx);
...
check('cards' in dbg, 'debug() exposes "cards"');
check('score' in dbg, 'debug() exposes "score"');
check('moves' in dbg, 'debug() exposes "moves"');
check('matched' in dbg, 'debug() exposes "matched"');
...
if (fail) { console.error('\n\nMEMORY GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nMEMORY GAMEPLAY PASS');
