// Targeted gameplay verify for GHOST MAZE.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/ghostmaze.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[ghostmaze gameplay]');
const g = RA.games.ghostmaze;
g.init();
RA.hideOverlay();
g.onStart();
...
// Ghostmaze: tap adjacent cells to move
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 200 }); }
  if (g.debug().started && !g.debug().over) {
    if (i % 8 === 0) RA.input.taps.push({ x: 100 + (i % 4) * 30, y: 200 });
  }
  g.update(dt);
  g.draw(RA.ctx);
}
...
check('ppos' in dbg, 'debug() exposes "ppos"');
check('ghosts' in dbg, 'debug() exposes "ghosts"');
check('score' in dbg, 'debug() exposes "score"');
check('over' in dbg, 'debug() exposes "over"');
check('started' in dbg, 'debug() exposes "started"');

// Phase 3: verify game handles a 1200-frame autopilot run without crashing
let crashed = false;
...
check(!crashed, '1200-frame autopilot run did not throw');

if (fail) { console.error('\n\nGHOSTMAZE GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nGHOSTMAZE GAMEPLAY PASS');
