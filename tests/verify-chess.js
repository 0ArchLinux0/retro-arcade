// Targeted gameplay verify for CHESS PUZZLE.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/chess.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[chess gameplay]');
const g = RA.games.chess;
g.init();
RA.hideOverlay();
g.onStart();
...
// Chess: tap to start, then tap a white piece + a target square
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 400 }); }
  if (g.debug().started && !g.debug().over) {
    if (i % 12 === 0) {
      // tap a white pawn and try to move it
      RA.input.taps.push({ x: 90, y: 480 });   // white pawn at e2 (approximate)
      RA.input.taps.push({ x: 90, y: 440 });   // move to e3
    }
  }
  g.update(dt);
  g.draw(RA.ctx);
...
check(Array.isArray(dbg.board), 'debug() exposes board[]');
check('turn' in dbg, 'debug() exposes "turn"');
...
if (fail) { console.error('\n\nCHESS GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nCHESS GAMEPLAY PASS');
