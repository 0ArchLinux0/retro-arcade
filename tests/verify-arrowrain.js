// Targeted gameplay verify for ARROW RAIN.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/arrowrain.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[arrowrain gameplay]');
const g = RA.games.arrowrain;
g.init();
RA.hideOverlay();
g.onStart();
...
// Arrowrain: hold + tap to fire
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 100 }); }
  if (g.debug().started && !g.debug().over) {
    RA.input.isDown = true;
    if (i % 8 === 0) RA.input.taps.push({ x: 180, y: 100 });
  }
  g.update(dt);
  g.draw(RA.ctx);
}
...
check('arrows' in dbg, 'debug() exposes "arrows"');
check('targets' in dbg, 'debug() exposes "targets"');
check('score' in dbg, 'debug() exposes "score"');
check('lives' in dbg, 'debug() exposes "lives"');
...
if (fail) { console.error('\n\nARROWRAIN GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nARROWRAIN GAMEPLAY PASS');
