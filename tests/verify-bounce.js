// Targeted gameplay verify for BOUNCE BALL.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/bounce.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[bounce gameplay]');
const g = RA.games.bounce;
g.init();
RA.hideOverlay();
g.onStart();
...
// Bounce: tap to launch ball, paddle follows finger
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 600 }); }
  if (g.debug().started) {
    // move paddle by wiggling input
    RA.input.x = 50 + (i * 5) % 260;
    RA.input.y = 600;
    if (i % 20 === 0) RA.input.taps.push({ x: 180, y: 600 });
  }
  g.update(dt);
  g.draw(RA.ctx);
}
...
check('paddle' in dbg, 'debug() exposes "paddle"');
check('balls' in dbg, 'debug() exposes "balls"');
check('bricks' in dbg, 'debug() exposes "bricks"');
...
if (fail) { console.error('\n\nBOUNCE GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nBOUNCE GAMEPLAY PASS');
