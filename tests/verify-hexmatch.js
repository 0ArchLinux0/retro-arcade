// Targeted gameplay verify for HEX MATCH.
...
load('js/core.js'); load('js/audio.js'); load('js/meta.js'); load('js/games/hexmatch.js');

let fail = 0;
function check(cond, msg) { if (cond) console.log('  PASS', msg); else { console.error('  FAIL', msg); fail++; } }

console.log('[hexmatch gameplay]');
const g = RA.games.hexmatch;
g.init();
RA.hideOverlay();
g.onStart();
...
// Hexmatch: tap to start, then tap hexes
for (let i = 0; i < 30; i++) {
  if (i === 0) { RA.input.taps.push({ x: 180, y: 200 }); }
  if (g.debug().started && !g.debug().over) {
    if (i % 6 === 0) {
      // tap a hex
      RA.input.taps.push({ x: 80 + (i % 6) * 40, y: 200 + ((i % 2) * 20) });
    }
  }
  g.update(dt);
  g.draw(RA.ctx);
...
check('board' in dbg, 'debug() exposes "board"');
...
if (fail) { console.error('\n\nHEXMATCH GAMEPLAY FAIL'); try { RA.audio.stopBGM(); } catch {} process.exit(1); }
console.log('\n\nHEXMATCH GAMEPLAY PASS');
